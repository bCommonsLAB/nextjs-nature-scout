...
- [x] **2.5 – Offline-Status-UI + mehrere Sessions.**
...
- [x] **2.6 – Aufräumen + „Offline-Daten bereinigen" im Profil.**
...
- [x] **3.1 – Web-App-Manifest + Icons.** Installierbar; Name/Theme/Icons; mobile-first.
- [x] **3.2 – Service Worker (App-Shell-Cache + Update-Strategie).**
  - App-Shell cachen; saubere Update-Strategie (keine veraltete App ausliefern).
- [x] **3.3 – iOS-Härtung + Tests.** _(Code-Härtung; Geräte-Tests durch Nutzer:in)_
...
| 2.5 | 2026-05-29 | _(dieser Commit)_ | **Orchestrator-Offline-Integration + Status-UI + mehrere Sessions + Resume aus IndexedDB.** `NatureScout`: Entwurf-Anlage online-first, bei offline/Server-Fehler **lokale Session** (IndexedDB); Auto-Save-Routing lokal vs. Server; Status „Lokal gesichert – wird übertragen, sobald online" + Retry; Resume lokaler Sessions über `?localSessionId=` (Metadaten + Bild-Vorschauen aus Blobs neu erzeugt). Bild-Aufnahme offline: `GetImage`/`SingleImageUpload` legen Blob lokal ab (volle Qualität, kein Upload/PlantNet), `clientImageId` je Slot; Context um `localSessionId` erweitert. „Meine Habitate": Bereich **Offline-Erfassungen** (Liste mit Status/Bildanzahl/Fehler, **Jetzt synchronisieren** via `useSync`, Fortsetzen/Löschen); Auto-Sync bei Empfang. **Abweichung:** Strategie `server-sofort` spiegelt (noch) nicht zusätzlich lokal – online ist bereits serverseitig abgesichert; lokaler Fallback greift bei offline/Server-Fehler. Offline-Verhalten ist nur am echten Gerät/Browser final prüfbar (Sandbox: tsc 98, lint Baseline, build Exit 0). |
| 2.6 | 2026-05-29 | _(dieser Commit)_ | **Aufräumen + „Offline-Daten bereinigen" im Profil.** Lokale Kopie wird **erst nach bestätigtem Abschluss** gelöscht: `Summary.tsx` ruft nach Server-2xx `deleteLocalSessionByJobId` (`offline/db.ts`) – nicht schon nach reinem Bild-Sync. Im Nutzerprofil neue Karte **Offline-Daten** (`components/natureScout/OfflineDataManager.tsx`): listet lokale Sessions (Standort, Bildanzahl, Status, belegter Speicher) und erlaubt gezieltes Löschen inkl. Blobs. `datenschutz-und-consent.md` ergänzt: Offline-Zwischenspeichern braucht **kein** gesondertes Consent (Consent beim Abschluss/Einreichen). **Phase 2 (Offline-Fähigkeit) damit abgeschlossen.** Verifikation: tsc 98, lint Baseline, build Exit 0. |
| 3.1 | 2026-05-30 | _(dieser Commit)_ | **Web-App-Manifest + Icons.** `src/app/manifest.ts` (Next liefert `/manifest.webmanifest`, auto-verlinkt): Name/Short-Name, `display: standalone`, `theme_color`, `background_color`, Icons 192/512/maskable. Icons aus dem Logo via `sharp` generiert (`public/icons/`), inkl. `apple-touch-icon`. Layout-Metadaten ergänzt (`applicationName`, `appleWebApp`, `icons`, `viewport.themeColor`, `viewportFit: cover`). Verifikation: tsc 98, lint Baseline, build Exit 0 (Manifest-Route vorhanden). |
| 3.2 | 2026-05-30 | _(dieser Commit)_ | **Service Worker (App-Shell-Cache + Update-Strategie).** `public/sw.js`: versionierter Cache, `activate` löscht alte Caches (keine veraltete App), `skipWaiting`+`clients.claim`; Navigationen **network-first** (frisch + offline-Fallback auf App-Shell), statische Assets **stale-while-revalidate**; `/api/*` wird **nie** gecacht (Auth/Upload/Draft). Registrierung `components/ServiceWorkerRegister.tsx` (nur Produktion), im Layout gemountet. → echtes „cold-offline" (App startet ohne Empfang), installierbar. |
| 3.3 | 2026-05-30 | _(dieser Commit)_ | **iOS-Härtung.** `requestPersistentStorage()` (`offline/storage.ts`) bittet um persistenten Speicher (reduziert iOS-Safari-Storage-Eviction); im Orchestrator beim Anlegen einer lokalen Session aufgerufen. iOS-Meta via `appleWebApp.capable`/`statusBarStyle`/`apple-touch-icon` + `viewportFit: cover` (3.1). **Geräte-Tests** (Flugmodus, Tab-Eviction, mehrere Sessions, Kamera/Datei-APIs in iOS-Safari) liegen bei der Nutzer:in – im Sandbox nicht durchführbar. **Phase 3 (PWA) damit abgeschlossen.** Verifikation: tsc 98, lint Baseline, build Exit 0. |
| Fix-1 | 2026-05-30 | _(dieser Commit)_ | **Resume-Sprung & verlorene Standortdaten behoben.** Bug (beim Testen gefunden): Fortsetzen (Server-Entwurf/lokale Session) sprang immer auf Schritt 1, die Karte zentrierte auf das aktuelle GPS und die Standortfelder gingen verloren. Fix: neues Modul `src/lib/erfassung-resume.ts` mit `inferResumeStep(metadata)` (Pflichtbilder/Analyse→8, Pflanzenbilder→6/7, Panorama→5, Polygon+Standortdaten→4, nur Polygon→3, Koordinaten→2, sonst 1; **nie >8**) + `hasSavedPolygon`. `NatureScout`: Entwurf-Resume nutzt `inferResumeStep(loadedMetadata)` (abgeschlossene Habitate starten weiter ab Schritt 1), lokaler Resume `session.aktiverSchritt` mit Fallback `inferResumeStep`; Standort-„Clear"-Effekt feuert nur noch beim bewussten Zurück **2→1** (via `prevAktiverSchrittRef`). `LocationDetermination`: gespeicherten Umriss aus Metadaten übernehmen, GPS-Auto-Zoom bei vorhandenem Polygon unterdrücken und stattdessen auf den gespeicherten Habitat-Stand zentrieren. Verifikation: tsc = Baseline (keine neuen Fehler), lint Baseline, build grün (Compile + 78/78 Seiten). |
| Fix-2 | 2026-05-30 | _(dieser Commit)_ | **Neue Erfassung nach Abschluss (HTTP 409) behoben.** Bug: „Neues Habitat" ließ die alte (abgeschlossene) `jobId` im Context → Auto-Save patchte einen Nicht-Entwurf → 409 „Nur Entwürfe können über diese Route aktualisiert werden". Fix: Context um `captureSessionKey` + `resetCaptureSession()` (leert `jobId`/`editJobId`/`localSessionId`, `key++`); `navigationbar.navigateToNewHabitat` und „Meine Habitate" (`startNewCapture`) rufen es vor der Navigation; `NatureScout` setzt bei `captureSessionKey`-Wechsel den Wizard zurück (`resetLocalCaptureWizard`); `activeDraftId` schließt Nicht-Entwürfe aus (`editJobId && isResumedDraft ? editJobId : ctxJobId`). Selbstheilend: `saveDraftNow` legt bei 409 einen neuen Entwurf an und wiederholt den PATCH einmal. Verifikation: tsc = Baseline, lint Baseline, build grün. |
| Fix-3 | 2026-05-30 | _(dieser Commit)_ | **Warm-Offline-Bild-Upload („Failed to fetch") behoben.** Bug: online gestartet (Server-`jobId`, **keine** lokale Session) → mitten in der Erfassung offline → `GetImage` lud weiter gegen `/api/upload`. Fix: `NatureScout.ensureLocalMirrorSession()` legt bei `strategy==='nur-lokal'` ohne lokale Session eine IndexedDB-Session **mit bestehender `jobId`** an (verhindert ein Sync-Duplikat) und wird an `SingleImageUpload`→`GetImage` durchgereicht; `GetImage.processImage` entscheidet per `checkOnline()` und legt offline den Blob lokal ab (`ensureLocalSession`-Fallback, kein Shadowing von `offlineSessionId`). Beim Verifizieren wurde zudem **1 vom vorherigen Stand eingeschleppte tsc-Regression** korrigiert (Rückgabetyp des `.then`-Callbacks im Retry-Button des Speicher-Status). Verifikation: tsc = Baseline, lint Baseline, build grün. **Browser-verifiziert** (Chrome, simuliertes Offline via `navigator.onLine`): Warm-Offline-Bild wird lokal in IndexedDB gespeichert (kein „Failed to fetch", keine Konsolenfehler), genau **1** Spiegel-Session mit mitgeführter `jobId`. Echte Cold-Offline-/PWA-Tests am Gerät (Flugmodus) weiterhin empfohlen. Die 409-Bedingung von Fix-2 wurde serverseitig bestätigt (PATCH `/draft` auf abgeschlossenes Habitat → 409). |
| Fix-4 | 2026-05-30 | _(dieser Commit)_ | **Resume-Karten-Zoom „ganz nahe" (Folgefehler aus Browser-Test).** Bug: Beim Öffnen eines bestehenden Standorts (Resume) zeigte die Karte den weiten Tal-Überblick (Zoom 13) statt nah am gespeicherten Umriss. Ursache: Die Zentrierung (`centerMap(lat,lng,20)`) lief, bevor die Leaflet-Karte real existierte (→ no-op); `mapInitializedRef` war kein verlässliches „Karte-bereit"-Signal, und der Effekt lief nicht erneut. Fix: `MapNoSSR` meldet via neuem Prop `onMapReady` die tatsächliche Karten-Erstellung (`isMapReady`); `LocationDetermination` zentriert erst bei `mapReady` (State statt Ref) auf den gespeicherten Habitat-Stand (Zoom 20) – greift auch, wenn Metadaten erst nach Mount geladen werden. Browser-verifiziert (Karte nun nah am Polygon, Standortdaten erhalten). Verifikation: tsc = Baseline, lint Baseline, build grün. |
| Fix-5 | 2026-05-30 | _(dieser Commit)_ | **Doppelte lokale Spiegel-Sessions verhindert (Folgefehler aus Browser-Test).** Bug: Beim Warm-Offline-Wechsel riefen Strategie-Effekt, Auto-Save und Bild-Upload `ensureLocalMirrorSession()` quasi gleichzeitig auf, bevor `ctxLocalSessionId` gesetzt war → **3 statt 1** lokale Sessions für dieselbe `jobId`; leere Sessions hätten beim Sync `bilder: []` patchen und das Bild überschreiben können (Datenverlust-Risiko). Fix: `ensureLocalMirrorSession` nutzt ein in-flight Promise (`localMirrorPromiseRef`), sodass gleichzeitige Aufrufer sich **eine** Anlage teilen; Ref wird per Effekt zurückgesetzt, sobald keine Session aktiv ist. Browser-verifiziert (jetzt genau 1 Session, `jobId` mitgeführt, Bild-Blob 4,8 MB lokal). Verifikation: tsc = Baseline, lint Baseline, build grün. |
